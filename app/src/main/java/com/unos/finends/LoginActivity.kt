    ...
    import com.unos.finends.features.bucket.editBucket.EditGoal
            ...
            NavHost(navController = navController, startDestination = "SplashScreen") {
                ...
                composable(Screen.Login.name) {
                    Login(
                        ...
                        onSignInClick = {
                            // Menjalankan login melalui email/password

                            // Menggunakan Google Sign-in jika diperlukan (jika menggunakan Google login)
                                        ...
                                        .addOnCompleteListener { task ->
                                            if (task.isSuccessful) {
                                                navController.navigate(Screen.Home.name) {
                                                    popUpTo(Screen.Login.name) { inclusive = true }  // Menghapus Login dari stack
                                                }
                                            } else {
    ...
    //                NavigationGraph(navController = navController)
                ...
                composable("AddNewGoal") {
                    AddNewGoal(navController = navController)
                }

                composable("AddSavingsScreen/{goalName}/{currentAmount}") { backStackEntry ->
                    val title = backStackEntry.arguments?.getString("goalName") ?: "Default Goal"
                    val currentAmount = backStackEntry.arguments?.getString("currentAmount") ?: "0"
                    var onSaveAmount = backStackEntry.arguments?.getString("currentAmount") ?: "0"
                    AddSavingsScreen(
                        navController = navController,
                        title = title,
                ...
                composable("edit_goal/{goalId}") { backStackEntry ->
                    ...
                    goalId?.let {
                        EditGoal(navController = navController)
                    }
                }
            }
        }
